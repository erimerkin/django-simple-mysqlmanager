...
DROP TABLE IF EXISTS `Given_At`;
CREATE TABLE `Given_At` (
    `course_id` varchar(64) NOT NULL,
    `classroom_id` varchar(64) NOT NULL,
...
DROP TABLE IF EXISTS `Has_Prerequisite`;
...
CREATE TRIGGER `ClassroomCapacityCheck`
...
CREATE TRIGGER `RemoveFromTaken` AFTER INSERT ON `Has_Grade` 
FOR EACH ROW 
BEGIN
    DELETE FROM Taken 
...
CREATE TRIGGER `StudentAutomator` AFTER INSERT ON `Has_Grade` 
FOR EACH ROW 
BEGIN
    UPDATE Students
...
CREATE TRIGGER `UpdateAfterGrade` AFTER UPDATE ON `Has_Grade` 
FOR EACH ROW 
BEGIN
    UPDATE Students
    SET Students.credits = (SELECT SUM(`credits`) FROM Course
...
CREATE TRIGGER `AfterDeleteGrade` AFTER DELETE ON `Has_Grade` 
FOR EACH ROW 
BEGIN
    UPDATE Students
    SET Students.credits = (SELECT SUM(`credits`) FROM Course
...
CREATE TRIGGER `AddCourseCheck` BEFORE INSERT ON `Taken` 
FOR EACH ROW 
BEGIN 
...
CREATE TRIGGER `CheckQuota` BEFORE INSERT ON `Taken` 
FOR EACH ROW 
BEGIN 
...
DROP TRIGGER IF EXISTS `CheckPrerequisites`;;
